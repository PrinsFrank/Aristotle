...
#[AsCommand('false', 'Mark a premise as false')]
class FalseCommand extends AristotleCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($identifier === null) {
            $output->writeln(Console::format('Please provide an identifier to mark as false.', Foreground::LightRed));
        ...
        if ($componentSet->hasModifierOfType($identifier, FalseModifier::class)) {
            $output->writeln('Premise has already been marked as false');

            return Command::SUCCESS;
        }

        $label = $input->getArgument('label');
        file_put_contents($this->configuration->getConfigPath(), PHP_EOL . 'false ' . $identifier . ($label !== null ? ' ' . $label : '') . PHP_EOL, FILE_APPEND);
