...
class PremiseCommand extends AristotleCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($existingIdentity !== null && $existingIdentity instanceof Premise === false) {
            $output->writeln('Identity "' . $identifier . '" already exists but is not a premise.');
        ...
        file_put_contents($this->configuration->getConfigPath(), PHP_EOL . 'premise ' . $identifier . ($label !== null ? ' "' . $label . '"' : '') . PHP_EOL, FILE_APPEND);
        $output->writeln('Added premise');
